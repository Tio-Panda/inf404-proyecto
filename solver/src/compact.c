...
static void
compact_queue (kissat * solver, links* links, queue* queue)
{
  LOG ("compacting queue");
  //printf("compact_queue!!!!!!!!!!!!!!!!!!!!!!!\n");
  //links *links = solver->links, *l;
  struct links* l;
  unsigned *p = &queue->first, prev = DISCONNECT;
  queue->stamp = 0;
  for (unsigned idx; !DISCONNECTED (idx = *p); p = &l->next)
	...
	{
	  queue->search.idx = midx;
	  queue->search.stamp = l->stamp;
	}
      *p = prev = midx;
    }
  queue->last = prev;
...
static bool
compact_unsatord (kissat * solver)
{
  LOG ("compacting list");
  int delete_node_num = 0;
  node* current_node = solver->unsatord_list.head;
  node* prev_node = NULL;
  ...
  while(current_node !=NULL) {
    idx = current_node->var_idx;
    midx = map_idx (solver, idx);
    
    if (midx == INVALID_IDX) {
      tmp = current_node->next; 
      delete_node(solver, &solver->unsatord_list, prev_node, current_node);
      current_node = tmp;
      delete_node_num++;
    }
    else {
      //printf("midx in backbone %d    ", midx);          
      current_node->var_idx = midx;
      prev_node = current_node;
      current_node = current_node->next;
    } 
  }

  solver->current_unsatord_node = solver->unsatord_list.head;

  if (delete_node_num > 0)
    return true;
  else
    return false;
}
...
kissat_finalize_compacting (kissat * solver, unsigned vars, unsigned mfixed)
{
  ...
  compact_queue (solver, solver->links, &solver->queue);
  if(GET_OPTION(neural_unsatord_stable_compact) ||
    GET_OPTION(neural_unsatord_focused_compact)) {
    if(compact_unsatord(solver))
      solver->unsatord_compacted = true;
  }
