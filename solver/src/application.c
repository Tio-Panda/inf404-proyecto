...
parse_options (application * application, int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
	...
	ERROR ("option '%s' only allowed as %s argument",
        ...
        //kissat_set_option (solver, "neural_unsatord", 1);
      }
#if !defined (NPROOFS) || !defined (_POSIX_C_SOURCE)
	...
	kissat_set_option (solver, "quiet", 1);
      else if (!strcmp (arg, "--neural_backbone_always"))
  ...
  kissat_set_option (solver, "neural_unsatord_stable", 1);
      else if (!strcmp (arg, "--random_phase_initial")) {
        kissat_set_option (solver, "random_phase_initial", 1);
      }
...
{
  file file;
  if (!application->backbone_path) {
    return true;
  }
  else if (!kissat_open_to_read_file (&file, application->backbone_path))
    ERROR ("failed to open '%s' for reading", application->backbone_path);

  kissat_parse_backbone (application->solver, &file, application->neuroback_cfd);
...
static bool
parse_unsatord (application * application)
{
  file file;
  if (!application->unsatord_path) {
    return true;
  }
  else if (!kissat_open_to_read_file (&file, application->unsatord_path))
    ERROR ("failed to open '%s' for reading", application->unsatord_path);

  kissat_parse_unsatord (application->solver, &file);
		 ...
		 int argc, char **argv, bool *cancel_alarm_ptr)
{
  *cancel_alarm_ptr = false;
  ...
  if (!parse_input (&application))
    ...
    if(!parse_backbone (&application))
      return 1;
    if(!parse_unsatord (&application))
      return 1;
